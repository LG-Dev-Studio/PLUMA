...
/**
 * `SensorCapacidades` es `final` (no mockeable) — este test compone una
 * instancia real sobre un `LenguajeInterface` mockeado (interfaz, sí
 * mockeable) y controla el resto vía `get_option`. El objetivo de este test
 * es la mecánica de persistencia/fallo-abierto, no valores concretos de
 * `HechosEntorno` (que dependen del runtime real de PHP).
 *
...
final class AlmacenPerfilEntornoTest extends CasoDePruebaUnitario {
	...
	private function almacen(): AlmacenPerfilEntorno {
		...
		$sensor = new SensorCapacidades( $proveedorLenguaje );
	...
	public function test_leer_con_json_corrupto_falla_abierto_y_refresca(): void {
		Functions\when( 'get_option' )->alias(
			...
			static fn ( string $opcion, $defecto = false ) => AlmacenPerfilEntorno::OPCION === $opcion
				? array( 'version' => '1.1' ) // faltan claves.
				: $defecto
		);
		Functions\expect( 'update_option' )->once()->andReturn( true );
