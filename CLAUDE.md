...
- El frontend público solo recibe: bloque del editor, schema JSON-LD, (opcional) banner de corrección, la página de autor por periodista (identidad editorial sintética, Art. 50 del Reglamento (UE) 2024/1689 — ver `docs/PLAN-MAESTRO-EVOLUCION.md`, Nivel Tres N.3), y el service worker + JS mínimo de suscripción a notificaciones push web (Nivel Cuatro W.3, Etapa 9 Porción 4 — decisión del propietario, 2026-07-28, que amplía esta lista de forma permanente porque W.3 exige registrar el service worker en el propio frontend, no hay forma de entregar push web sin él). Peso adicional en frontend ≈ 0 salvo el service worker de W.3, que se sirve solo al lector que se suscribe explícitamente (opt-in), nunca a todo visitante. Prohibido encolar assets de admin fuera de las pantallas de PLUMA.
